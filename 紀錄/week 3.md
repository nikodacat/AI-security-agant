...
## 觀察/思考
  ...
  將CVE資料庫和通用攻擊/防禦方式拿來finetune，將產出模型用於產生對於攻擊手段的防禦手法，但因為CVE是一個持續更新的資料庫，因此會逐漸過時，也有可能在第一時間錯過大漏洞，只要想辦法改成以外接CVE資料庫api的話，應該會比較好，但就要改變這個架構，或是在需要時調度相關的CVE。
